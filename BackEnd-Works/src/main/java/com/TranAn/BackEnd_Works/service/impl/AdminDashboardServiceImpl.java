...
public class AdminDashboardServiceImpl implements AdminDashboardService {
        ...
        private DashboardStatsResponseDto.JobStats getJobStats() {
                ...
                // Đếm theo level — 1 query GROUP BY thay vì 5 query COUNT riêng lẻ
                Map<String, Long> levelMap = new java.util.HashMap<>();
                jobRepository.countGroupByLevel().forEach(row -> levelMap.put(row[0].toString(), (Long) row[1]));

                Long internJobs = levelMap.getOrDefault("INTERN", 0L);
                Long fresherJobs = levelMap.getOrDefault("FRESHER", 0L);
                Long middleJobs = levelMap.getOrDefault("MIDDLE", 0L);
                Long seniorJobs = levelMap.getOrDefault("SENIOR", 0L);
                Long leaderJobs = levelMap.getOrDefault("LEADER", 0L);
        ...
        private DashboardStatsResponseDto.CompanyStats getCompanyStats() {
                ...
                List<DashboardStatsResponseDto.CompanyJobCount> topCompanies = companyRepository
                                .findTopCompaniesByJobCount()
                                .stream()
                                .limit(10)
        ...
        private DashboardStatsResponseDto.TopPerformers getTopPerformers() {
                ...
                List<DashboardStatsResponseDto.CompanyResumeCount> topCompaniesByResumes = companyRepository
                                .findTopCompaniesByResumeCount()
                                .stream()
                                .limit(10)
                ...
                List<DashboardStatsResponseDto.JobResumeCount> topJobsByResumes = jobRepository
                                .findTopJobsByResumeCount()
                                .stream()
                                .limit(10)
        ...
        private List<DashboardStatsResponseDto.MonthlyData> getLast6MonthsData(String type) {
                ...
                for (int i = 5; i >= 0; i--) {
                        ...
                        Instant endOfMonth = yearMonth.atEndOfMonth().atTime(23, 59, 59).atZone(ZoneId.systemDefault())
                                        .toInstant();
